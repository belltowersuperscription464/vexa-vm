(() => {
  ...
  async function loadSettings() {
    try {
      ...
      const canManageBcp38 = allows("network:write");
      $("#bcp38-enabled")?.toggleAttribute("disabled", !canManageBcp38);
      $("#save-bcp38")?.toggleAttribute("disabled", !canManageBcp38);
      $("#host-network-security-panel")?.classList.toggle("opacity-60", !canManageBcp38);
      try {
      ...
      } catch (error) { setText("#bcp38-state", error.message); }
  ...
  async function initSettings() {
    ...
    $("#save-bcp38")?.addEventListener("click", async () => { const toggle = $("#bcp38-enabled"); const enabled = Boolean(toggle?.checked); if (enabled && toggle?.dataset.original !== "true") { const approved = await confirmAction({ title: "Enable hypervisor BCP38 filtering?", message: "Guest packets with source addresses not assigned in Vexa-VM will be dropped. This is host-only and may add packet-filtering cost.", confirmLabel: "Enable BCP38" }); if (!approved) { toggle.checked = false; return; } } try { await api("/api/v1/network/security", { method: "PATCH", body: { bcp38_enabled: enabled } }); toast(`BCP38 ${enabled ? "enabled" : "disabled"}`, "success"); await loadSettings(); } catch (error) { toast(error.message, "error", error.requestId); } });
