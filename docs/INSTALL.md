...
curl -fsSL https://raw.githubusercontent.com/ItzGlace/vaxa-vm/main/install.sh | \
  sudo VEXA_VERSION=v0.1.1 VEXA_BIND=127.0.0.1:8080 VEXA_PUBLIC_URL=https://vm.example.com bash
```
...
sudo install -Dm0755 target/release/vexa-vm /opt/vexa-vm/releases/0.1.1/bin/vexa-vm
sudo install -Dm0755 target/release/vexa-update-helper \
  /opt/vexa-vm/releases/0.1.1/bin/vexa-update-helper
sudo ln -s releases/0.1.1 /opt/vexa-vm/current
```
